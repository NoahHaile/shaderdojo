...
INSERT INTO lesson (
    ...
    3,
    'Cell colours',
    '<p>Track which neighbour was closest and assign it a hue. Reuse the <code>hsv2rgb</code> helper from the Colors course.</p>',
    'attribute vec4 aVertexPosition;
void main() { gl_Position = aVertexPosition; }',
    'float rand(vec2 p) {
    return fract(sin(dot(p, vec2(12.9898, 78.233))) * 43758.5453);
...
INSERT INTO lesson (
    ...
    0,
    'A single octave',
    '<p>Start with the 2D <code>noise</code> function from the Noise course. Render it once at frequency 4 &mdash; soft, low-detail.</p>
