...
((SELECT id FROM course WHERE slug = 'hsv-color'), 'uQitrBNNf48', 3,
 'Polar wheel that breathes',
 '<p>You will drive all three HSV knobs at once. Hue spins around the center. Saturation grows from the middle out. Value pulses with time. The result is a breathing color wheel.</p><p>The starter has the salmon disc from last lesson. It uses the centered position <code>p</code> for saturation. You will bring in the other two knobs.</p><p>Hue comes from the polar angle, like the polar lesson:</p><p><code>float h = atan(p.y, p.x) / (2.0 * 3.14159) + 0.5;</code></p><p>Value comes from time, the same wave from Family A:</p><p><code>float v = 0.5 + 0.5 * sin(u_time);</code></p><p>Then call <code>hsv2rgb(vec3(h, s, v))</code> with all three knobs in motion. The canvas turns into a color wheel that brightens and dims as a whole.</p><p>This is what HSV is good for. Each knob does its own thing. You drove three sources together and the colors stay clean.</p><p>Reference: <a href="https://thebookofshaders.com/06/" target="_blank" rel="noreferrer">Book of Shaders, Colors</a>.</p>',
 'vec3 hsv2rgb(vec3 c) {
    vec4 K = vec4(1.0, 2.0/3.0, 1.0/3.0, 3.0);
    vec3 p = abs(fract(c.xxx + K.xyz) * 6.0 - K.www);
    return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
